...
impl AgentScene {
    ...
    pub fn scene_prompt(&self) -> &'static str {
        match self {
            ...
            Self::SpeedUp => "当前场景为「电脑变快」。请帮助用户诊断电脑卡顿、开机缓慢和资源占用问题，优先关注正在运行的高占用进程、开机启动项，也可以补充视觉效果、电源计划和磁盘占用这类常见原因。开始时先查看整体 CPU、内存、磁盘和电源计划；再根据情况列出占用较高的进程，以及开机启动项。不要结束进程或直接修改开机项，只给出建议。遇到用途不明确的程序时，可以联网检索其常见用途和关闭风险。对于系统关键进程、驱动、安全软件以及电源或热管理相关服务，不要写入建议清单。在收尾阶段，如果已经形成较稳定的建议，请调用清单写入工具把建议保存到任务：进程项必须包含 name、path、reason，path 尽量使用扫描结果里的完整路径；开机项必须包含 name、location、command、reason，location 必须原样复制启动项列表里的位置代码，例如 hkcu_run。同一软件如果既在运行又开机自启，应分别写成一条进程建议和一条开机建议。写入完成后，告知用户建议已保存到「任务」，用户可以在任务页面用开关禁用开机项，或结束仍在运行的进程。不要提及工具名称或任何内部组件名称。",
    ...
    pub fn enabled_tools(&self) -> &'static str {
        match self {
            ...
            Self::SpeedUp => "get_system_perf,list_processes,list_startup_items,write_speed_up_checklist,file_read,http_request",
            Self::General => "*",
        }
    }
}
